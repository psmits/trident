...
\begin{equation}
  \begin{aligned}
    ...
    a_{w, p} &\sim \mathrm{N}(\delta_{p}, \sigma_{a}) \\
...
%\section{Variance Partioning and Intraclass Correlation Coeficient}
%
%There are multiple levels and sources of variance in this model. Each of the grouping factors account for some amount of variance in the data. Normalizing that variance by the total variance from all sources, we can see the percent contribution to variance. 
%
%``level 1'' is the ``data level.'' in a regular linear regression problem, this would be the residual variance. that's not calculated in logistic regression, which poses a problem. how do we get this value? simulation?
%
%``level 2'' is the ``time level.'' the logistic regression coefficients \textbf{vary} through time, we are actually estimating that variance in \(\Sigma_{B}\) and \(\tau_{B}\). there is also the non-nested age-based varying-intercept. this source of variance is in \(\Sigma_{a}\) and \(\tau_{a}\).
%
%``level 3'' is the ``phyla level.'' the over-time averages of the regression coefficients further vary between phyla. that variance is part of \(\Sigma_{\alpha}\) and \(\tau_{\alpha}\). there is also the average age effect on extinction probability which is part of \(\Sigma_{\delta}\) and \(\tau_{\delta}\).
%
%that's a lot of variance. 
%
%we want to figure out what the total variance is, so we can determine the percent contribution for each level and segment of variance. the level 1 issue and the nonnestedness pose potential challenges.
