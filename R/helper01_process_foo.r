...
prepare_analysis <- function(x, fg = NULL) {
  ...
  tb <- tb %>%
    ...
    mutate_at(.vars = vars(mst, ncell, latext, maxgcd, area, temp),
              .funs = ~ scale(log1p(.x))) %>%
    group_by(fullname) %>%
    mutate(diff1_mst = c(0, diff(mst)),
           diff2_mst = ifelse(n() > 1,
                              c(rep(0, 2), diff(mst, lag = 2)),
                              rep(0, n())),
           diff3_mst = ifelse(n() > 2,
                              c(rep(0, 3), diff(mst, lag = 3)),
                              rep(0, n())),
           diff4_mst = ifelse(n() > 3,
                              c(rep(0, 4), diff(mst, lag = 4)),
                              rep(0, n())),
           ...
           diff1_maxgcd = c(0, diff(maxgcd)),
           diff2_maxgcd = ifelse(n() > 1,
                                 c(rep(0, 2), diff(maxgcd, lag = 2)),
                                 rep(0, n())),
           diff3_maxgcd = ifelse(n() > 2,
                                 c(rep(0, 3), diff(maxgcd, lag = 3)),
                                 rep(0, n())),
           diff4_maxgcd = ifelse(n() > 3,
                                 c(rep(0, 4), diff(maxgcd, lag = 4)),
                                 rep(0, n())),
           ...
           diff1_latext = c(0, diff(latext)),
           diff2_latext = ifelse(n() > 1,
                                 c(rep(0, 2), diff(latext, lag = 2)),
                                 rep(0, n())),
           diff3_latext = ifelse(n() > 2,
                                 c(rep(0, 3), diff(latext, lag = 3)),
                                 rep(0, n())),
           diff4_latext = ifelse(n() > 3,
                                 c(rep(0, 4), diff(latext, lag = 4)),
                                 rep(0, n())),
           ...
           diff1_area = c(0, diff(area)),
           diff2_area = ifelse(n() > 1,
                               c(rep(0, 2), diff(area, lag = 2)),
                               rep(0, n())),
           diff3_area = ifelse(n() > 2,
                               c(rep(0, 3), diff(area, lag = 3)),
                               rep(0, n())),
           diff4_area = ifelse(n() > 3,
                               c(rep(0, 4), diff(area, lag = 4)),
                               rep(0, n())),
           ...
           diff1_temp = c(0, diff(temp)),
           diff2_temp = ifelse(n() > 1,
                               c(rep(0, 2), diff(temp, lag = 2)),
                               rep(0, n())),
           diff3_temp = ifelse(n() > 2,
                               c(rep(0, 3), diff(temp, lag = 3)),
                               rep(0, n())),
           diff4_temp = ifelse(n() > 3,
                               c(rep(0, 4), diff(temp, lag = 4)),
                               rep(0, n())),
